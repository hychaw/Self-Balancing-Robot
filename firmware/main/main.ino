...
const int PWM_MIN = 0;  
float imbalanceMultiplier = 1.00;
/*
...
float PC_KP = 0.0015; // 0.001 ; 0.0015
float PC_KD = 0.00075;  // 0.00125; 0.002 
...
int rampStateCounter = 0;

int turnDirection = 0;
int TURN_WAIT_TIME = 3500;
unsigned long turnStartTime;

int RAMP_WAIT_TIME = 10000;
...
void setup() {
    ...
    //debug for motor imbalance
    /*setLeftMotor(250);
    setRightMotor(250);
    delay(2000);  // let them stabilize
    // Read encoder counts over a fixed time window
    noInterrupts();
    long L0 = LEncCount;
    long R0 = REncCount;
    interrupts();
    delay(2000);
    noInterrupts();
    long L_count = LEncCount - L0;
    long R_count = REncCount - R0;
    interrupts();
    setLeftMotor(0);
    setRightMotor(0);
    computeEncoderValues();
    Serial.print("L counts: "); Serial.print(L_count);
    Serial.print("  R counts: "); Serial.println(R_count);
    Serial.print("Ratio (L/R): "); Serial.println((float)L_count / R_count);*/
...
void loop() {
    ...
    if ((PC_tNow - PC_tLast) > PC_LOOP_DELAY*1000) {
      ...
      if (fabs(targetAngle) < MAX_ANGLE && ((fabs(PC_KI*positionIntegral)) < integralWeight*MAX_ANGLE *0.95)) {
...
/*
      static unsigned long stationarySince = 0;
      const float STATIONARY_VELOCITY_THRESHOLD = 50.0;
      const unsigned long STATIONARY_DURATION_MS = 2000;
      
      if ((fabs(positionDerivative) < STATIONARY_VELOCITY_THRESHOLD) && currentCommand == 2) {
          if (stationarySince == 0) {
              stationarySince = millis();
          } else if (millis() - stationarySince > STATIONARY_DURATION_MS) {
              // Robot has been still for 2s during a stop command
              // Accept current encoder position as the new home
              targetPositionSmoothed = currentPosition;
              lastTargetSmoothed = currentPosition;
              targetPosition = currentPosition;
              positionIntegral *= 0.5;  // partially discharge integral
              stationarySince = 0;
          }
      } else {
          stationarySince = 0;  // any motion resets the timer
      }*/
    }
  ...
  if(loopsPassed % 2 == 0){ 
    if(zAngle > targetHeading + 2){     //adding margin to prevent oscillation
      if (turning) {
        headingCorrection = 8;
      } else {
        headingCorrection = 3;
      }
    } else if (zAngle < targetHeading - 2){
        if (turning) {
          headingCorrection = -8;
        } else {
          headingCorrection = -3;
    ...
    if (commandChanged) {
      switch(currentCommand) {
        case 3: //left turn
          ...
          turnState = add45; // start turn sequences
          break;
    ...
    */
    ...
    switch (turnState) {
      case turnIdle: 
        ...
        imbalanceMultiplier = 1.00;
        break;
      case add45:
        targetHeading += 45*turnDirection;
        imbalanceMultiplier = 1.07;
        turnState = turnWait;
        turnStartTime = millis();
        break;
      case turnWait:
        if (millis() - turnStartTime >= TURN_WAIT_TIME) {
          if (fmod(targetHeading, 90.0) == 0) {
            turnState = turnIdle;
            currentCommand = 8;     //dummy command for init reasons
          } else turnState = add45;
          turnStartTime = millis();
        }
        break;
    ...
    switch(rampState) {
      ...
      PC_KP = 0.0015; // 0.001 ; 0.0015
      PC_KD = 0.0015;  // 0.00125; 0.002 
      PC_KI = 0.0003; //0.0003 ; 
      integralWeight = 1.0;
      //restore defaults. Return after complete
        break;
        case rampClimb:
          TARGET_SLEW_RATE = 600.0;
          targetPosition = 4000;
          integralWeight = 1.0;
          MAX_ANGLE = 12.0;
          // PC_KD = 0.002;
          //if not helpful, Kd to 0.004.
          ...
          break;
        case rampClimb10:
          TARGET_SLEW_RATE = 600;
          targetPosition = 7000;
          if (millis() - rampStartTime >= RAMP_WAIT_TIME + 3000) {
            rampState = rampWait;
            rampStartTime = millis();
          }
          break;
        case rampClimb15:
          ...
          targetPosition = 9000;
          MAX_ANGLE = 15.0;
          if (millis() - rampStartTime >= RAMP_WAIT_TIME + 5000) {
            rampState = rampWait;
            rampStartTime = millis();
          }
          break;
        case rampWait:
          if (millis() - rampStartTime >= 8000) {
        ...
        case rampDescend: 
          targetPosition = 0;
          if (millis() - rampStartTime >= RAMP_WAIT_TIME) {
            switch(rampStateCounter) {
              case 0: 
                rampStateCounter++;
                rampState = rampClimb10;
                break;
              case 1:
                rampStateCounter++;
    ...
    switch (currentCommand) {
      ...
      case 1:           // backwards      
        ...
        homeSet = false;
        targetPosition = 0;
        //NOTE: If it is permissable to hit STOP between forward and back tests, this is acceptable. If not,
        // change targetPosition = 0. Seek prof/TA guidance
        targetHeading = 0;
        break;
      case 2:           // stop
        MAX_ANGLE = 3.0;
    ...
    int rightPWM = applyDeadband((basePWM - headingCorrection)*imbalanceMultiplier, R_DEADBAND);
