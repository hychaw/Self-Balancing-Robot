    ...
    */
    ...
    switch(rampState) {
      ...
      //do nothing and restore stable max angle. Return after complete
        break;
        case rampClimb:
          TARGET_SLEW_RATE = 1700.0;
          targetPosition = 3900;
          integralWeight = 0.5;
          MAX_ANGLE = 16.0;

          PC_KI = 0.0003;
          integralCLAMPER = 21000;
        
          if ((LEncCount+REncCount/2.0) > 3900) {
            rampState = rampWait; //REVERT
            rampStartTime = millis();
          }
          break;
          
        case rampClimb10:
        integralCLAMPER = 24300;

        if((LEncCount+REncCount/2.0) > 7400){
            rampState = rampClimb15;
          }
        
        /*if(((LEncCount+REncCount/2.0) > 4000) && ((LEncCount+REncCount/2.0) < 4700)){
            extraAngle = -1.7;
          }
          else if((LEncCount+REncCount/2.0) > 4700)
          extraAngle = -3.0;*/
